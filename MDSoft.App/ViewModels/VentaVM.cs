...
namespace Tracking.ViewModels
{
    public partial class VentaVM : ObservableObject
    {
        ...
        {
            var encontrado = DetalleVenta.FirstOrDefault(dv => dv.Producto.ProId == result.ProId);
            if (encontrado == null)
            {
                DetalleVenta.Add(new DetalleVentaDTO
                {
                    Producto = result,
                    Cantidad = 1,
                    Total = 1 * result.ProPrecio.Value
                });
                MostarTotal();
            }
        ...
        private void DisminuirEvent(DetalleVentaDTO detalle)
        {
            ...
            {
                detalle.Cantidad -= 1;
                detalle.Total = detalle.Producto.ProPrecio.Value * detalle.Cantidad   ;
                MostarTotal();
            }
        ...
        private async Task FinalizarVenta()
        {
            ...
            try
            {
                ...
                foreach (var item in DetalleVenta)
                {
                    ...
                    {
                        IdProducto = item.Producto.ProId,
